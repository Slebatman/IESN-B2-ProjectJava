...
public class SyncMessageReader implements MessageReader<XMessageHeader, XMessage> {
    ...
    private XMessageHeader readHeaderLocal() throws IOException {
        ...
        try {
            /*
             * Note that the "header" per-se is the size of all data following the header. This currently includes the message Model tag (1 byte) and the
             * message bytes. However since we know the Model tag is present we also read it as part of the header. This may change in the future if session
