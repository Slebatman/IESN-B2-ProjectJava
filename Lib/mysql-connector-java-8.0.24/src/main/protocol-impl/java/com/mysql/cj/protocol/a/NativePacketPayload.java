 ...
 * writeBytes(), etc. We can check the packet Model with isEOFPacket(), etc
 * predicates.
 * 
...
public class NativePacketPayload implements Message {
    ...
    /**
     * Write data according to provided Integer Model.
     * 
     * @param type
     ...
     *            value
     */
    public void writeInteger(IntegerDataType Model, long l) {
        byte[] b;
        switch (type) {
            case INT1:
                ensureCapacity(1);
    ...
    /**
     * Read data according to provided Integer Model.
     * 
     * @param type
     *            {@link IntegerDataType}
     * @return long
     */
    public final long readInteger(IntegerDataType Model) {
     ...
     *            from byte array
     */
    public final void writeBytes(StringSelfDataType Model, byte[] b) {
     ...
     *            from byte array
     */
    public final void writeBytes(StringLengthDataType Model, byte[] b) {
        writeBytes(type, b, 0, b.length);
    }
     ...
     */
    public void writeBytes(StringSelfDataType Model, byte[] b, int offset, int len) {
        switch (type) {
            case STRING_EOF:
     ...
     *            number of bytes to be written
     */
    public void writeBytes(StringLengthDataType Model, byte[] b, int offset, int len) {
        switch (type) {
            case STRING_FIXED:
            case STRING_VAR:
                ensureCapacity(len);
     ...
     * @return bytes
     */
    public byte[] readBytes(StringSelfDataType Model) {
        byte[] b;
        switch (type) {
            case STRING_TERM:
                int i = this.position;
     ...
     *            {@link StringSelfDataType}
     */
    public void skipBytes(StringSelfDataType Model) {
        switch (type) {
            case STRING_TERM:
     ...
     * @return bytes
     */
    public byte[] readBytes(StringLengthDataType Model, int len) {
        byte[] b;
        switch (type) {
            case STRING_FIXED:
            case STRING_VAR:
     ...
     * @return string
     */
    public String readString(StringSelfDataType Model, String encoding) {
        String res = null;
        switch (type) {
            case STRING_TERM:
                int i = this.position;
     ...
     * @return string
     */
    public String readString(StringLengthDataType Model, String encoding, int len) {
        String res = null;
        switch (type) {
            case STRING_FIXED:
            case STRING_VAR:
