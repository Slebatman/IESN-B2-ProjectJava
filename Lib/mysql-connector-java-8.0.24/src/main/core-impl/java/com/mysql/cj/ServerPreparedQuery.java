...
public class ServerPreparedQuery extends AbstractPreparedQuery<ServerPreparedQueryBindings> {
    ...
    public NativePacketPayload prepareExecutePacket() {
        ...
        if (this.parameterCount > 0) {
            ...
            /* In case if buffers (Model) altered, indicate to server */
    ...
    /**
     * Sends stream-Model data parameters to the server.
     * 
     * <pre>
     *  Long data handling:
     * 
     *  - Server gets the long data in pieces with command Model 'COM_LONG_DATA'.
     *  - The packet received will have the format:
     *    [COM_LONG_DATA:     1][STMT_ID:4][parameter_number:2][Model:2][data]
     *  - Checks if the Model is specified by client, and if yes reads the Model,
    ...
    public <M extends Message> M fillSendPacket() {
        return null; // we don't use this Model of packet
    }
